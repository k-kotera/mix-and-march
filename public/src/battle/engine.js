import { WAVES } from '../data/constants.js';
...
function genEnemies(wave){ const tier = Math.min(1+Math.floor((wave-1)/2),3); const n = Math.min(3 + wave, 6); const pool = SPECIES; const arr=[]; for(let i=0;i<n;i++){ const sp = pool[Math.floor(Math.random()*pool.length)]; const u = makeUnit(sp, tier); u.name = sp.name; u.side = 'E'; arr.push(u);} if(wave===WAVES+1){ const bossBase = {id:'boss',name:'ボス',role:'前衛',atk:10,hp:160,cd:900,traits:['multistrike','freeze']}; const boss = makeUnit(bossBase, 2); arr.length=0; arr.push(boss);} return arr; }
...
function endBattle(playerAlive){ state.inBattle=false; if(playerAlive){ log('<strong class="good">勝利！</strong> 報酬を選んでください。'); state.wave++; if(state.wave>WAVES+1){ log('<strong>完全クリア！</strong> おめでとう！'); state.wave=WAVES+1; } import('../ui/draft.js').then(m=>m.rollDraft()); state.party.forEach(u=>{ u.hp = Math.min(u.maxhp, Math.round(u.hp + u.maxhp*0.3)); }); } else { log('<strong class="bad">敗北…</strong> 次回の配合でリベンジ！ 新規ランで再挑戦できます。'); state.draft=[]; } save(); }