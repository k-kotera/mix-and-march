...
function newRun(){
  ...
  state.party.push(makeUnit(SPECIES[Math.floor(Math.random()*SPECIES.length)],1));
  log('<span class="good">新規ラン開始！</span> 報酬を選ぶ→配合/強化（1回）→バトルの流れです。');
  rollDraft(); renderAll(); save();
}
function startOrNext(){ if(state.inBattle) return; if(state.draft.length>0){ log('<span class="bad">先に報酬を選んでください</span>'); return; } startBattle(); }
window.addEventListener('keydown',(e)=>{ if(e.key===' '){ e.preventDefault(); startOrNext(); } if(e.key==='r' || e.key==='R'){ e.preventDefault(); newRun(); } });
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('btnSpeed').onclick = ()=>{ state.speed = state.speed===1?2:1; renderAll(); };
